...
void adMCMCgp(int niter, int thin, int n, int nparmg, double *parmg, int phi, double rho, double psi, int nblocks, int **blocks, int *blocks_size, double **mu, double ***S, double theta, double eta, double iota, double delta, double *acpt_target,
              double acpt_hptarget, double acpt_lpsitarget, double decay, int refresh, int refreshhp, int refreshlpsi,
              double *lm, double lmhp, double lmpsi, double temp,
              int *refresh_counter, int refreshhp_counter, int refreshlpsi_counter, int verbose, int ticker,
              double *parmgsamp, double *acptsamp, double *lpsamp, double *lprsamp, double *lkappasamp, int *phisamp, double *lpsisamp, double *acpthpsamp, double *acptlpsisamp, double *lmglhsamp, double *usamp);
...
double ltCopDen(int p, double *x, int phi, double rho, double lpsi);
double lprior(int p, int phi, double lkappa, double ltau, double lpsi);
...
double psiFn(double z){
	return 18.0 * plogis(z, 0.0, 1.0, 1, 0) + 2.0;
}

double psiFn_inv(double psi){
	return qlogis((psi - 2.0) / 18.0, 0.0, 1.0, 1, 0);
}
...
void BJQRtp(double *parmg, int *phiVar, double *rhoVar, double *psiVar, double *phigrid, double *xVar, double *yVar, int *status, double *weights, int *toShrink, double *hyper, int *dim, double *gridpars,
            double *tauG, double *muVar, double *SVar, double *thetaVar, double *etaVar, double *iotaVar, double *deltaVar, int *blocksVar, int *blocks_size, double *dmcmcpar, int *imcmcpar,
            double *parmgsamp, double *acptsamp, double *lpsamp, double *lprsamp, double *lkappasamp, int *phisamp, double *lpsisamp, double *acpthpsamp, double *acptlpsisamp, double *lmglhsamp, double *usamp, int *distribution){
	...
	wt = weights;
	int phi = phiVar[0];
	double rho = rhoVar[0], psi = psiVar[0];
	...
	int refreshhp = imcmcpar[4], refreshhp_counter = imcmcpar[5], refreshlpsi = imcmcpar[6], refreshlpsi_counter = imcmcpar[7];
	int *refresh_counter = imcmcpar + 8;

	double temp = dmcmcpar[0], decay = dmcmcpar[1], acpt_hptarget = dmcmcpar[2], lmhp = dmcmcpar[3], acpt_lpsitarget = dmcmcpar[4], lmpsi = dmcmcpar[5];
	double *acpt_target = dmcmcpar + 6, *lm = acpt_target + nblocks;
	...
	double theta = thetaVar[0], eta = etaVar[0], delta = deltaVar[0], iota = iotaVar[0];

	//Call to adaptive MCMC code
	adMCMCgp(niter, thin, n, nparmg, parmg, phi, rho, psi, nblocks, blocks, blocks_size, mu, S, theta, eta, iota, delta, acpt_target, acpt_hptarget, acpt_lpsitarget,
	         decay, refresh, refreshhp, refreshlpsi, lm, lmhp, lmpsi, temp,
	         refresh_counter, refreshhp_counter, refreshlpsi_counter, verbose, ticker, parmgsamp,
	         acptsamp, lpsamp, lprsamp, lkappasamp, phisamp, lpsisamp, acpthpsamp, acptlpsisamp,
	         lmglhsamp, usamp);
}
...
//      logpostFn(par0, 1.0, 0, llvec, pgvec, y, cens, lp, rpvec);
//      // Rprintf("lp = %g\n", lp[0]);
//      corInvChol(n, lphi, Cinvchol_init);
//      result = lgauCopDen(n, rpvec, Cinvchol_init) + lp[0];
//      // Rprintf("res = %g\n", result);
//      return result;
// }
//
...
//      double result;
//      corInvChol(n, lphi, Cinvchol_init);
//      result = lgauCopDen(n, rpvec, Cinvchol_init) + lprior(n, lphi, Cinvchol_init);
//      return result;
// }
//
// void INIT_COP(double *par, double *xVar, double *yVar, double *distVar, double *kappaVar, int *status, double *weights, int *toShrink, double *hyper, int *dim, double *gridpars, double *tauG, double *siglim, int *covfun, int *distribution,
//   double *lphiVar, double *decaypar){
//
//      int i, j, k, l;
//
//      int reach = 0;
...
//      n = dim[reach++]; p = dim[reach++]; L = dim[reach++]; mid = dim[reach++];
//      m = dim[reach++]; ngrid = dim[reach++]; nkap = dim[reach++];
//      dist = distribution[0]; kernel = covfun[0]; kappa = kappaVar[0];
...
//      diagcop = vect(n);
//      Cinvchol_init = mymatrix(n, n);
//      distance = distVar;
//   decay_lb = decaypar[0];
//   decay_ub = decaypar[1];
//   decay_len = decay_ub - decay_lb;
//   double lmglh, *rpvec_cur = vect(n);
//      sig_pos = (m+1)*(p+1);                                                                                   //position in par of sigma
//      par0 = par;
//
//      double *sig_opt = vect(2), *lphi_opt = vect(2);
//      int iopt = 0, ioptnew = 1;
//
//      sig_opt[iopt] = siglim[0];
//      lphi_opt[iopt] = phiFn_inv(decay_lb+1.0);
//      sig_opt[ioptnew] = siglim[1];
//      lphi_opt[ioptnew] = phiFn_inv(decay_ub-1.0);
//
//      double lphi = phiFn_inv((decay_lb+decay_ub)/2.0);
//      double sig_a = siglim[0], sig_b = siglim[1];
//
//      // Rprintf("lphi = %g\n", lphi);
//      // Rprintf("sig_a = %g\n", sig_a);
//      // Rprintf("sig_b = %g\n", sig_b);
//
//      double fsiga = lpost1(sig_a, yVar, cens, lphi);
//      double fsigb = lpost1(sig_b, yVar, cens, lphi);
//      Rprintf("\n");
//      double lphi_a = phiFn_inv(decay_lb+1.0), lphi_b = phiFn_inv(decay_ub-1.0);
//      double fphia = lpost2(lphi_a);
//      double fphib = lpost2(lphi_b);
//
//      while( !Stopping_Rule(sig_opt[iopt], sig_opt[ioptnew], 1.0e-5) || !Stopping_Rule(lphi_opt[iopt], lphi_opt[ioptnew], 1.0e-5)) {
//
//              // Use Max Golden Section Search algorithm to maximize log likelihood over sigma
//              Max_Search_Golden_Section1(lpost1, &sig_a, &fsiga, &sig_b, &fsigb, yVar, cens, lphi, 1.0e-5);
//              sig_opt[ioptnew] = (sig_a + sig_b) / 2.0;
//
//              par[sig_pos] = sig_opt[ioptnew];
//              logpostFn(par, 1.0, 0, llvec, pgvec, yVar, cens, lp, rpvec);
//     lmglh = lp[0];
//     for(i = 0; i < n; i++) rpvec_cur[i] = rpvec[i];
//              lphi_a = phiFn_inv(decay_lb+1.0), lphi_b = phiFn_inv(decay_ub - 1.0);
//              fphia = lpost2(lphi_a);
//              fphib = lpost2(lphi_b);
//
//              Max_Search_Golden_Section2(lpost2, &lphi_a, &fphia, &lphi_b, &fphib, 1.0e-5);
//              lphi_opt[ioptnew] = (lphi_a + lphi_b) / 2.0;
//
//              iopt = ioptnew;
//              ioptnew = !iopt;
//
//              lphi = lphi_opt[iopt];
//              sig_a = siglim[0], sig_b = siglim[1];
//              fsiga = lpost1(sig_a, yVar, cens, lphi);
//              fsigb = lpost1(sig_b, yVar, cens, lphi);
//
//              corInvChol(n, lphi, Cinvchol_init);
//              Rprintf("lpost = %g\n", lmglh + lgauCopDen(n, rpvec_cur, Cinvchol_init) + lprior(n, lphi, Cinvchol_init));
//     // Rprintf("lpost = %g\n", lgauCopDen(n, rpvec_cur, Cinvchol_init));
//     // Rprintf("lpost = %g\n", lmglh);
//     // Rprintf("lpost = %g\n", lprior(n, lphi, Cinvchol_init));
//
//      }
//
//      par[sig_pos] = sig_opt[iopt];
//      lphiVar[0] = lphi_opt[iopt];
// }
//
//
// void Max_Search_Golden_Section1( double (*f)(double, double*, int*, double), double* a, double *fa, double* b, double* fb, double *y, int *cens, double lphi, double tolerance){
...
//      x1 = *b - lambda * (*b - *a);
//      x2 = *a + lambda * (*b - *a);
//      fx1 = f(x1, y, cens, lphi);                                                                                         //pointer to input function
//      fx2 = f(x2, y, cens, lphi);                                                                                         //pointer to input function
//
//      // If tolerance specificed as negative, set to default, minimal tolerance
//      if (tolerance <= 0.0) tolerance = 1.0e-5 * (*b - *a);
//
...
void adMCMCgp(int niter, int thin, int n, int nparmg, double *parmg, int phi, double rho, double psi,
              int nblocks, int **blocks, int *blocks_size, double **mu, double ***S, double theta,
              double eta, double iota, double delta, double *acpt_target, double acpt_hptarget,
              double acpt_lpsitarget, double decay, int refresh, int refreshhp, int refreshlpsi,
              double *lm, double lmhp, double lmpsi, double temp, int *refresh_counter,
              int refreshhp_counter, int refreshlpsi_counter, int verbose, int ticker, double *parmgsamp,
              double *acptsamp, double *lpsamp, double *lprsamp, double *lkappasamp, int *phisamp,
              double *lpsisamp, double *acpthpsamp, double *acptlpsisamp, double *lmglhsamp,
              double *usamp){
...
// copula part
	...
	int psi_size = 0;
	double acpt_lpsi = 0.0, lpsibar = 0.0, alphalpsi, fraclpsi = sqrt(1.0 / ((double)refreshlpsi_counter + 1.0));
	...
	double lp_diff, lmglh, lmglhnew, lcop, lcopnew, lpr, lprnew, lkappanew, kappasq, rhonew, lpsi, lpsinew;
	...
	lcop = ltCopDen(n, U[ipar], phi, rho, lpsi);
	lpr = lprior(n, phi, lkappa, parmgstore[ipar][nparmg - 2], lpsi);
...
// Rprintf("Initial lrho = %g\n", lrho);
	...
	int iter, store_lp = 0, store_parmg = 0, store_acpt = 0, store_lmglh = 0, store_lpr = 0, store_lkappa = 0, store_phi = 0, store_lpsi = 0, store_acpthp = 0, store_acptlpsi = 0, store_u = 0;
	double *alpha_run = vect(nblocks), alphahp_run = 0.0, alphalpsi_run = 0.0, chs, lambda;
	int *run_counter = ivect(nblocks), runhp_counter = 0, runlpsi_counter = 0;
...
// joint part parameters
	...
	for(iter = 0; iter < niter; iter++) {
		for(b = 0; b < nblocks; b++) {
			...
			lcopnew = ltCopDen(n, U[iparnew], phi, rhonew, lpsi);
			lprnew = lprior(n, phi, lkappa, parmgstore[iparnew][nparmg - 2], lpsi);
		...
		lkappanew = lkappa + lmhp*zsamp[0]*eta*sqrt(3.0 / rgamma(3.0, 1.0));
		...
		lcopnew = ltCopDen(n, rpvec, phi, rhonew, lpsi);
		lprnew = lprior(n, phi, lkappanew, parmgstore[ipar][nparmg - 2], lpsi);
		...
		for(i = 0; i < nphi; i++) {
			lprgrid[i] = lprior(n, i, lkappa, parmgstore[ipar][nparmg - 2], lpsi);
			lcopgrid[i] = ltCopDen(n, U[ipar], i, rho, lpsi);
		...
		for(i = 0; i < n; i++) U[iparnew][i] = rpvec[i];
		lcopnew = ltCopDen(n, U[iparnew], phi, rhonew, lpsi);
		lprnew = lprior(n, phi, lkappanew, parmgstore[iparnew][nparmg - 2], lpsi);
		lp_diff = lmglhnew - lmglh + lcopnew - lcop + lprnew - lpr;
		...
		psi_size++;
		zsamp[0] = rnorm(0.0, 1.0);
		lpsinew = lpsi + lmpsi*zsamp[0]*delta*sqrt(3.0 / rgamma(3.0, 1.0));

		lcopnew = ltCopDen(n, U[ipar], phi, rho, lpsinew);
		lprnew = lprior(n, phi, lkappa, parmgstore[ipar][nparmg - 2], lpsinew);

		// Rprintf("lgtr = %g", lgtrstore[iparrnew[g]][g]);
		// Rprintf("lcopnew = %g", lcopnew[g]);
		// Rprintf("lprnew = %g", lprnew[g]);
		// Rprintf("\n");
		lp_diff = lcopnew - lcop + lprnew - lpr;
		alphalpsi = exp(lp_diff); if(alphalpsi > 1.0) alphalpsi = 1.0;
		if(runif(0.0, 1.0) < alphalpsi) {
			lcop = lcopnew;
			lpr = lprnew;
			lpsi = lpsinew;
		}
		alphalpsi_run = ((double)runlpsi_counter * alphalpsi_run + alphalpsi) / ((double)(runlpsi_counter + 1.0));
		runlpsi_counter++;
		...
		if((iter + 1) % thin == 0) {
			...
			lpsisamp[store_lpsi++] = lpsi;
			acptlpsisamp[store_acptlpsi++] = alphalpsi;
		...
		chs = (double) psi_size; if(chs < 1.0) chs = 1.0;
		acpt_lpsi += (alphalpsi - acpt_lpsi) / chs;
		lpsibar += (lpsi - lpsibar) / chs;
		if(psi_size == refreshlpsi) {
			refreshlpsi_counter++;
			fraclpsi = sqrt(1.0 / ((double)refreshlpsi_counter + 1.0));
			delta = sqrt((1.0 - fraclpsi) * delta * delta + fraclpsi * (lpsibar - iota) * (lpsibar - iota));
			iota += fraclpsi * (lpsibar - iota);
			lpsibar = 0.0;
			lmpsi *= exp(fraclpsi * (acpt_lpsi - acpt_lpsitarget));
			acpt_lpsi = 0.0;
			psi_size = 0;
		}
	}
...
double lprior(int p, int phi, double lkappa, double ltau, double lpsi){
	...
	int i;
	double sigsq = kapsq + tausq, rho = kapsq/sigsq, psi = psiFn(lpsi), elpsi = exp(lpsi);

	for(i = 0; i < p; i++) result += log1p(rho*(Lphigrid[phi][i]-1.0));
	// result = -result/2.0 + lkappa - exp(lkappa);
	result = -result/2.0 + (arho - 1.0) * log(rho) + (brho - 1.0) * log(1.0 - rho) - 2.0 * log(sigsq) + log(kapsq) + log(tausq);
	// result += lgammafn((psi + p)/ 2.0) + p * (lgammafn(psi / 2.0) - lgammafn((psi + 1.0) / 2.0)) - lgammafn(psi / 2.0) - 2.0 * log(psi) + lpsi - 2.0 * log(1.0 + elpsi);
	result += lgammafn((psi + p)/ 2.0) + p * (lgammafn(psi / 2.0) - lgammafn((psi + 1.0) / 2.0)) - lgammafn(psi / 2.0) + lpsi - 2.0 * log(1.0 + elpsi);
	return result;
}
...
double wtsumsquares(double *x, int phi, double rho, int n){
	...
	for(i = 0; i < n; i++)
		wss += x[i] * x[i] / (rho*(Lphigrid[phi][i] - 1.0) + 1.0);
	return wss;
}

double ltCopDen(int p, double *x, int phi, double rho, double lpsi){
	int i;
	double result = 0.0, psi = psiFn(lpsi);

	for(i = 0; i < p; i++) zcop[i] = qt(x[i], psi, 1, 0);
	mvprod(Gphigrid[phi], zcop, zprod, p, p, 1);
	// Rprintvec("zprod = ", "%0.4f ", zprod, p);
	// Rprintvec("Gphi = ", "%0.4f ", Gphigrid[phi][0], p);

	for(i = 0; i < p; i++) result += log(1.0 + zcop[i] * zcop[i] / psi);
	result = result * (psi + 1.0) / 2 - (psi + p) * log(1.0 + wtsumsquares(zprod, phi, rho, p)/psi)/2;
