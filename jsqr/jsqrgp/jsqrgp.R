...
qrjointgp <- function(x, y, location = NULL, distance = NULL, nphi, kappa, kernel = c("se", "matern"), nsamp = 1e3, thin = 10, cens = NULL,
                       ...
                       ref.size = 3, ref.hpsize = 3, blocking = "std5", temp = 1, expo = 2,
  ...
  imcmc.par <- c(nblocks, ref.size, verbose, max(10, niter/1e4), ref.hpsize, 0, rep(0, nblocks))
  dmcmc.par <- c(temp, 0.999, acpt.hptarget, 2.38, rep(acpt.target, nblocks), 2.38 / sqrt(blocks.size))
  ...
  rho <- 0.5           # alpha in the paper
  phi <- floor(nphi/2)
  
  theta <- 0
  eta <- 1
  
  tm.c <- system.time(oo <- .C("BJQRgp", par = as.double(parmg), phi = as.integer(phi), rho = as.double(rho), phigrid = as.double(sapply(eigen.grid, function(u) cbind(u$val, u$vec))), 
                               ...
                               tau.g = as.double(tau.g), muV = as.double(blocks.mu), SV = as.double(blocks.S), thetaVar = as.double(theta), etaVar = as.double(eta),
                               blocks = as.integer(blocks.ix), blocks.size = as.integer(blocks.size), dmcmcpar = as.double(dmcmc.par), imcmcpar = as.integer(imcmc.par), parmgsamp = double(nsamp * nparmg),
                               acptsamp = double(nsamp * nblocks), lpsamp = double(nsamp), lprsamp = double(nsamp), lkappasamp = double(nsamp), phisamp = integer(nsamp), acpthpsamp = double(nsamp),
                               lmglhsamp = double(nsamp), usamp = double(nsamp*n), fbase.choice = as.integer(fbase.choice)))
