...
predict.qrjointgp <- function(object, x.test, location.test, tau.test, burn.perc = 0.5, nmc = 5e2){
  ...
  coefgp <- coef.qrjointgp(object, plot=FALSE, nmc = nmc, burn.perc = burn.perc, reduce = FALSE)
  ...
  betahat <- function(k, n.loc, n.col, len.tau.comp){
    ...
    for(i in 1:n.loc){
      for(l in 1:n.col){
        beta.hat[i,l,] <- approx(x = tau.post[k,i,], y = coefgp$beta.samp[,l,k], xout = tau.test)$y
      }
    }
    return(beta.hat)
  }
...
waicgp <- function(object, burn.perc = 0.5, nmc = 200){
  nsamp <- object$dim[11]                      # number of iterations retains in sample
  pars <- matrix(object$parmgsamp, ncol = nsamp) # reorder parameters into npar x nsamp matrix
  ss <- unique(round(nsamp * seq(burn.perc, 1, len = nmc + 1)[-1])) # indices over which to summarizing (exclude burn; keep nmc samples)
  ss.len <- length(ss)
  ...
  rhosamp <- (exp(object$lkappasamp)/exp(matrix(object$parmgsamp, ncol = nsamp)[nparmg - 1,]))[ss]
  phisamp <- (object$phisamp + 1)[ss]
  wsamp <- epssamp <- matrix(NA, nrow = n, ncol = ss.len)
  for(i in 1:ss.len){
    Omega <- solve(Sigmainvgrid[phisamp[i],,] + diag(rhosamp[i]/(1 - rhosamp[i]), n))
    Omega <- (Omega + t(Omega))/2
    wsamp[,i] <- rmvnorm(1, mean = sqrt(rhosamp[i])/(1 - rhosamp[i])*Omega%*%zsamp[,i], sigma = Omega)
    epssamp[,i] <- (zsamp[,i] - sqrt(rhosamp[i])*wsamp[,i])/sqrt(1 - rhosamp[i])
    ll[,i] <- ll[,i] - log(sqrt(1 - rhosamp[i])*dnorm(zsamp[,i])/dnorm(epssamp[,i]))
  }
