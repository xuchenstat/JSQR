...
qrkb.test.loss <- qrkb.test.qt <- true.test.qt <- array(NA, dim = c(100, n.test, tau.len))
...
for(datanum in 101:200){
  ...
  true.test.qt[datanum - 100,,] <- qt.test
  ...
  qrkb.test.qt[datanum - 100,,] <- apply(coef.kb, 1, function(u) as.matrix(cbind(1,x.test))%*%u)
  qrkb.test.loss[datanum - 100,,] <- abs(qrkb.test.qt[datanum - 100,,] - qt.test)
