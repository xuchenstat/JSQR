...
b0.prime <- function(tau){3/dt(qt(tau, df = 2), df = 2)}
...
for (datanum in 601:700){
  ...
  jsqr.fit <- qrjointgp(x, y, par = 'prior', distance = distance, nphi = 10, kernel = "matern", acpt.hptarget = 0.15, acpt.target = 0.15,  kappa = 2, nsamp = 2e3, thin = 10, fbase = 't')

  save(jsqr.fit, file = paste('jsqrgpfit', datanum, '.RData', sep = ''))
}