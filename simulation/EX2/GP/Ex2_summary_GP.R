...
for(datanum in datanums){
  filename <- paste('jsqrfit', datanum + 300, '.RData', sep = '')
  load(filename)
...
library(qrjoint)
for(datanum in datanums){
  filename <- paste('jqrfit', datanum + 300, '.RData', sep = '')
  load(filename)
...
for(datanum in datanums){
  ...
  filename <- paste('kbfit', datanum + 300, '.RData', sep = '')
  load(filename)
...
for(datanum in datanums){
  coef.lq <- coef.uq <- matrix(NA, nrow = length(tau.grid), ncol = p+1)
  filename <- paste('alpfit', datanum + 300, '.RData', sep = '')
  load(filename)
...
for(datanum in datanums){
  ...
  filename <- paste('aldfit', datanum + 300, '.RData', sep = '')
  load(filename)
...
save(mean.abs.error, file = 'ex2gpmae.RData')
save(cp, file = 'ex2gpcp.RData')
