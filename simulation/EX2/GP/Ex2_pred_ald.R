...
ald.test.loss <- ald.test.qt <- array(NA, dim = c(100, n.test, tau.len))
...
for(datanum in 301:400){
  ...
  file.name <- paste('aldfit', datanum, '.RData', sep = '')
  load(file.name)
  
  coef.ald <- matrix(NA, nrow = tau.len, ncol = p+1)
  
  for(i in 1:tau.len){
    coef.ald[i,] <- apply(ald.fit[[i]]$betadraw[501:1000,], 2, mean)
  }
  
  ald.test.qt[datanum - 300,,] <- apply(coef.ald, 1, function(u) as.matrix(cbind(1,x.test))%*%u)
  ald.test.loss[datanum - 300,,] <- abs(ald.test.qt[datanum - 300,,] - qt.test)
